...
class UnityGameContainer : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        world = new World(10,10,10000,worldRenerer);

        e = new Entity(Position.one * 400, name: "Player");
        world.AddEntity(e, Position.zero, Position.zero, true);
        world.AddEntity(new Entity(Position.one * 400, name: "Rock"), Position.zero, Position.down * 400, true);
    ...
    private void FixedUpdate()
    {
        ...
        if(input != Vector2.zero)
            e.MoveInLine(new Position((int)input.x, (int)input.y), 10, world, true);
        input = Vector2.zero;
